...
backends = [
    ["libsnark",    "my_pysnark.libsnark.backend"],
    ["libsnarkgg",  "my_pysnark.libsnark.backendgg"],
    ["qaptools",    "my_pysnark.qaptools.backend"],
    ["snarkjs",     "my_pysnark.snarkjsbackend"],
    ["zkinterface", "my_pysnark.zkinterface.backend"],
    ["nobackend",   "my_pysnark.nobackend"]
]
...
def snark(fn):
    def snark__(*args, **kwargs):
        ...
        argscopy = [for_each_in(int, lambda x: PubVal(x), args[0]), for_each_in(int, lambda x: PrivVal(x), args[1])]
